...
UINT ThreadWaitNotepad(LPVOID pParam) // 스레드 선언 함수( 반환형 및 매개변수 타입은 필수로 유지해야 함)
{
	...
	if (::ShellExecuteEx(&sei))
	{
		...
		DWORD dwResult = ::WaitForMultipleObjects(2, arhList, FALSE, INFINITE); // 메모리 누수 방지를 위함 (메모장 실행 중 MFC를 종료하면 메모리에 남는걸 방지)
		...
		{
			OutputDebugString(_T("WorkderThreadDemo 예제가 종료되었습니다. \n"));
		}
	}
...
void CWorkerThreadDemoDlg::OnBnClickedButtonRunnotepad()
{
	...
	if (g_pThread != NULL) // 중복 스레드 실행 방지
	{
		AfxMessageBox(_T("메모장 감시 스레드가 이미 실행중입니다."));
		return;
	}
	g_pThread = AfxBeginThread(ThreadWaitNotepad, NULL); // 스레드를 생성
	if (g_pThread == NULL)
	{
