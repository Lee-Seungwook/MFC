...
DllMain(HINSTANCE hInstance, DWORD dwReason, LPVOID lpReserved) // dll을 로드하거나 언로드할때 호출되는 함수
{
	// lpReserved를 사용하는 경우 다음을 제거하십시오.
	UNREFERENCED_PARAMETER(lpReserved);

	if (dwReason == DLL_PROCESS_ATTACH) // 라이브러리가 로드되는 시점
	{
...
// _declspec(dllexport) // 외부로 익스포트되어 사용하는 함수라는 것을 명시
BOOL WINAPI GetModulePath(CString& strPath)
{
	TCHAR szBuffer[MAX_PATH];
	::ZeroMemory(szBuffer, sizeof(szBuffer));
	::GetModuleFileName(NULL, szBuffer, MAX_PATH); // 실행 바이너리 파일의 실제 경로를 알아내서 버퍼에 저장하는 역할
