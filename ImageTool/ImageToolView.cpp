...
void CImageToolView::OnDraw(CDC* pDC)
{
	...
	POSITION posStraight = pDoc->m_MyStraightList.GetHeadPosition();
	while (posStraight!= NULL)
	{
		CMyStraight *pMyStraight = pDoc->m_MyStraightList.GetNext(posStraight);
	}
...
void CImageToolView::OnLButtonDown(UINT nFlags, CPoint point)
{
	...
	if (m_nLine == TRUE)
	{
		...
		// CTypedPtrList 클래스의 AddTail() 함수를 호출하여 CMyData 객체 추가
	...
	{
		CImageToolDoc *pDoc = GetDocument();
		// 마우스가 눌린 순간에 new 연산자를 이용하여 새로운 곡선을 저장할 CMyStraight 객체를 생성
		m_pCurrentMyStraight = new CMyStraight(m_color, m_nWidth, m_nStyle);
		// CTypedPtrList 클래스의 AddTail() 함수를 호출하여 CMyStraight 객체 추가
		pDoc->m_MyStraightList.AddTail(m_pCurrentMyStraight);
		// 도큐먼트 데이터가 변경되었음을 알리기 위한 SetmodifiedFlag() 함수 호출
		pDoc->SetModifiedFlag();
		// 마우스 커서가 다른 윈도우 위로 이동해도 메시지를 계속 잡아 올 수 있도록 함
		SetCapture();
